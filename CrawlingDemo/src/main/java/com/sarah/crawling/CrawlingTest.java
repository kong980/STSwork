...
public class CrawlingTest {
	...
	//		Elements imageUrlElements = doc.getElementsByClass("cl_top"); //회사명 & 채용공고 제목 가져오기 위함
...
//				System.out.println(goSiteUrlElements.get(i).text());
				System.out.print(companyElements.get(j).getElementsByClass("cl_top").select("a").text() + " : "); // 회사명 텍스트만 뽑기
				System.out.println(titleElements.get(j).getElementsByClass("cl_top").select("a").text()); // 채용공고 텍스트만 뽑기
				System.out.println(titleElements.get(j).getElementsByClass("cl_md").select("span").text()); // 조건만 뽑기
				System.out.println(dateElements.get(j).getElementsByClass("cl_btm").select("span").get(0).text()); // 조건만 뽑기
				
				final String siteUrl = linkElements.get(j).attr("abs:href"); // 채용공고 세부 페이지 url만 뽑기
